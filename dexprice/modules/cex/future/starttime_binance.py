

import dexprice.modules.cex.future.ovhl_binance as ovhl_binance
import dexprice.modules.utilis.timedefine as timedefine

def determine_initial_timesta(symbol,port=7890):
    interval = "1MON"
    start = None  # 允许为空
    # end is the now time
   # end = 1738308461  # 允许为空
    end = None
   # end = int(time.time())
    kline_data = ovhl_binance.get_kline_data(symbol, interval, start, end,port)
    time = []
    for i in kline_data:
        time.append(i[0])


    if (len(time) <= 2000):
        start = time[0]//1000
        #here we find the start month
        #and we need find the start time
        end = timedefine.addtime(start)
        interval = '1D'
        kline_data = ovhl_binance.get_kline_data(symbol, interval, start, end,port)
        real_start = kline_data[0][0]//1000
        time  = timedefine.timestamp_to_datetime(real_start)
        return time
    else:
        print(" to old we  delete")
        return None


if __name__ == "__main__":
    symbol = "BTC_USDT"
    time = determine_initial_timesta(symbol,7890)
    if(time):
        print(time)